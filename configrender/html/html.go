...
func (f *Formatter) writeHTML(w io.Writer, style *chroma.Style, tokens []chroma.Token) (err error) { // nolint: gocyclo
	...
	for index, tokens := range lines {
		// meow: figure out current yaml field name with nesting
		var lineName string
		switch tokens[0].Type {
		case chroma.NameTag:
			nameStack = append(nameStack[:0], tokens[0].Value)
			lineName = tokens[0].Value
		case chroma.Whitespace:
			depth := len(tokens[0].Value) / 4
			if len(tokens) > 1 && tokens[1].Type == chroma.NameTag {
				nameStack = append(nameStack[:depth], tokens[1].Value)
				lineName = strings.Join(nameStack, ".")
			}
		}
		...
		if !(f.preventSurroundingPre || f.inlineCode) {
			// Start of Line
			fmt.Fprint(w, `<span`)
			fmt.Fprint(w, f.lineIDAttribute(line))
			...
			// Line number
			fmt.Fprintf(w, "<span%s>%s</span>", f.styleAttr(css, chroma.LineNumbers), f.lineTitleWithLinkIfNeeded(css, lineDigits, line))
		...
		for i, token := range tokens {
			...
			// meow: link to yaml field name
			if token.Type == chroma.NameTag && (i == 0 || i == 1) {
				html = fmt.Sprintf(`<a class="name-link" id="%s" href="#%s">%s</a>`, lineName, lineName, html)
			}
			fmt.Fprint(w, html)
		}
